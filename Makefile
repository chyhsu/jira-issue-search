...
external-srv:
	...
	kubectl -n q2 port-forward ollama-7c8f77c44f-fkcr6 11434:11434 > /dev/null 2>&1 &

clean-external-srv:
	kill `ps | grep 'nats-server -js' | grep -v grep | awk '{print $$1}'` > /dev/null 2>&1 &
	kill `ps | grep 'kubectl -n q2 port-forward ollama-7c8f77c44f-fkcr6 11434:11434' | grep -v grep | awk '{print $$1}'` > /dev/null 2>&1 &